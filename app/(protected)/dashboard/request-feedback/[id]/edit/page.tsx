...
export default async function EditFeedbackRequestPage({ params }: Props) {
  ...
  if (!editableStatuses.includes(pr.status)) {
    return redirect(`/dashboard/request-feedback/${id}`);
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <Link
        href={`/dashboard/request-feedback/${id}`}
      ...
      <Card className="border-gray-700 bg-gray-800">
        <CardHeader>
          <CardTitle className="text-xl">
            {isDraft ? "Edit Draft" : "Edit Feedback Request"}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <FeedbackRequestForm
